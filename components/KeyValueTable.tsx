...
interface KeyValueTableProps {
  ...
  /** Keys that cannot be deleted (delete button is hidden) */
  lockedKeys?: string[];
}
...
export default function KeyValueTable({
  ...
  const lockedSet = new Set(lockedKeys.map(k => k.toLowerCase()));
  ...
  return (
    <div className="flex flex-col gap-1.5">
      ...
      {pairs.map(pair => {
        const isLocked = lockedSet.has(pair.key.toLowerCase());
        return (
          <div key={pair.id} className={`grid gap-2 items-center ${showEnabled ? 'grid-cols-[16px_1fr_1fr_28px]' : 'grid-cols-[1fr_1fr_28px]'}`}>
            ...
            <input
              ...
              readOnly={isLocked}
              className={`bg-[#161b27] border border-slate-700/60 rounded-lg px-3 py-1.5 text-sm text-slate-200 placeholder-slate-600 focus:outline-none focus:border-indigo-500/60 focus:ring-1 focus:ring-indigo-500/20 font-mono ${!pair.enabled && showEnabled ? 'opacity-40' : ''} ${isLocked ? 'text-indigo-300/70 cursor-default' : ''}`}
            />
            <input
              type="text"
              value={pair.value}
